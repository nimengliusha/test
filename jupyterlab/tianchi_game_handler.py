...
class TianchiGameBuilder(CommonBuilder):
    building = False
...
class TianchiGameHandler(APIHandler):
    ...
    @gen.coroutine
    def get(self):
        self.log.warn('enter TianchiGameHandler')
        id = self.get_argument('id', None)
        self.log.warn('uid is %s', id)
        ...
        data = yield self.builder.send_request(self.url + '/games/' + id + '/detail', user_number, ticket, None, cookies=self.cookies)
        ...
        if not files['result']:
            ...
            return
        self.finish(json.dumps(files['result']))


# The path for lab build.
tianchi_game_path = r"/lab/api/tianchi/game/detail"
