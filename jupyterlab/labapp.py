...
class LabApp(NotebookApp):
    ...
    * App mode: JupyterLab allows multiple JupyterLab "applications" to be
    ...
    # added by liujiong for allow origin
    ...
    if 'dsw_env' in os.environ:
        if os.environ['dsw_env'] == 'dev':
            jinja_template_vars = {
                'env_url': 'http://g.alicdn.daily.taobao.net/cdn-versions/alishu-dsw-jupyterlab/cn-beijing-daily/index.js',
                'timestamp': int(round(time.time() * 1000))
            }
            # enable jupyterlab in iframe
            tornado_settings = {
                'headers': {
        ...
        elif os.environ['dsw_env'] == 'pre':
            jinja_template_vars = {
                'env_url': 'https://g.alicdn.com/cdn-versions/alishu-dsw-jupyterlab/cn-beijing-pre/index.js',
                'timestamp': int(round(time.time() * 1000))
            }
            # enable jupyterlab in iframe
            tornado_settings = {
        ...
        elif os.environ['dsw_env'] == 'prod':
            jinja_template_vars = {
                'env_url': 'https://g.alicdn.com/cdn-versions/alishu-dsw-jupyterlab/cn-beijing-prod/index.js',
                'timestamp': int(round(time.time() * 1000))
            }
            tornado_settings = {
                'headers': {
